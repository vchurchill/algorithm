...
def M2L(x1,y1,x2,y2,rb,p,N):
  ...
  # N is the truncation number for the Fourier series
  ...
  # kernel matrices
  K1 = np.zeros(shape=(p,p,N))
  K2 = np.zeros(shape=(p,p,N))
  ...
  for i in range(0,p):
    for j in range(0,p):
      for k in range(-N,N):
        K1[i,j,k] = LaplaceF2D(rq1[i],zq1[i],rq2[j],zq2[j],k)
        K3[i,j] = K3[i,j] + K1[i,j,k]
