...
def checkpotential(N,p,rb,x,y):
  ...
  # move the sources to the box I want
  sources = sources + [x, y]
  ...
  for i in range(0,p):
      rt[i] = radius * math.cos(math.pi*2 * i/p) + c[0]
      zt[i] = radius * math.sin(math.pi*2 * i/p) + c[1]
